...
Genius.prototype.getArtistInfoByName = function getArtistInfoByName(artistName) {
...
Genius.prototype.getArtistIdByName = function getArtistIdByName(artistName) {
  return this.getArtistInfoByName(artistName)
    .then(artistInfo => ({ artistId: artistInfo.id, artistName }))
}

Genius.prototype.getArtistUrlByName = function getArtistUrlByName(artistName) {
  return this.getArtistInfoByName(artistName)
    .then(artistInfo => artistInfo.url)
}
...
function extractRelevantSongFields(response) {
  return {
    ...
    url: response.url,
    song_art_image_thumbnail_url: response.song_art_image_thumbnail_url || response.header_image_thumbnail_url,
    id: response.id,
...
Genius.prototype.getSongsByPopularity = function getSongsByPopularity(artistId, amountOfSongs) {
...
Genius.prototype.extractSongsWithValidDate = function extractSongsWithValidDate(response, sinceDate) {
  return new Promise(res => res(response))
  .then(filterSongList)
  .then(songs => songs.map(song => extractRelevantSongFields(song)))
  .then(songs => Promise.all(songs.map(song => this.getSongLyrics(song.url).then(songDetails => ({ ...song, ...songDetails })))))
  .then(mergedSongsDetails => mergedSongsDetails.filter(song => song.releaseDate.isValid() && song.releaseDate.isAfter(sinceDate)))
}

Genius.prototype.getSongsSinceDate = function getSongsSinceDate(artistId, sinceDate) {
  const allSongs = []
  const iteratePage = page => this.songsByArtist(artistId, {
    per_page: 10,
    page,
  })
    .then(handleErrors)
    .then((response) => {
      ...
      return this.extractSongsWithValidDate(response, sinceDate).then(newSongs => ({ songs: newSongs, nextPage }))
    })
    .then(({ songs, nextPage }) => {
      allSongs.push(...songs)
...
function parseSongHTML(htmlText) {
  ...
  } else {  // try to get release date from album release year
    releaseDate = $('.song_album-info-release_year').text().replace(/[\D]*/g, '')
    if (releaseDate) releaseDate = moment(`31-12-${releaseDate}`, 'DD-MM-YYYY')  // set it to end of year for comparison reasons
    else releaseDate = moment.invalid()
  }
  return {
    lyrics,
    releaseDate,
...
function extractRelevantReferentFields(response, song, artistName) {
  return {
...
Genius.prototype.getSongReferents = function getSongReferents(song, artistName) {
  const songId = song.id
  return this.referents({ song_id: songId }, {
    per_page: 50,
  })
  .then(handleErrors)
  .then(response => response.referents.map(referent => extractRelevantReferentFields(referent, song, artistName)))
}
...
Genius.prototype.getRandomSongLines = function getRandomSongLines(song, artistName) {
  return this.getSongLyrics(song.url)
  .then(({ lyrics }) => {
    let lines = extractRandomLinesFromLyrics(lyrics)
    lines = lines.map(line => extractRelevantReferentFields(line, song, artistName))
    return lines
  })
}
