...
function saveLine(line) {
  const { text, url, artist, songTitle, album } = line
  const newLine = new Line({
    text,
    artist,
    songTitle,
...
function scrapeSong(song, artistName) {
  ...
  genius.getSongReferents(song, artistName)
  .then((referents) => {
    const multiLineReferentsThreshold = 2
    const multiLineReferents = referents.filter(ref => ref.text.includes('\n'))
    if (multiLineReferents.length >= multiLineReferentsThreshold) return multiLineReferents
...
routes.post('/scrape/popular', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  console.log('/scrape/popular', artistNames, artistNames.length, numberOfSongsToParse)

  for (let i = 0; i < artistNames.length; i += 1) {
    const artist = artistNames[i]
    genius.getArtistIdByName(artist)
    .then(({ artistId }) => genius.getSongsByPopularity(artistId, numberOfSongsToParse))
    .then((songs) => {
...
routes.post('/scrape/date', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  if (!sinceDate.isValid()) {
    res.status(400).json(createError('Please pass a _valid_ "timestampToParseFrom" parameter.'))
    return
  }
